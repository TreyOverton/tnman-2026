# RGT OPORD 26-05-01

> **Source files:**
> - [RO-26-05-01-final.docx](/docs/source/RO-26-05-01-final.docx) — **final issued version** (authoritative)
> - [RO-26-05-01-final-draft.docx](/docs/source/RO-26-05-01-final-draft.docx) — final coordinating draft
> - [RO-26-05-01-revised.docx](/docs/source/RO-26-05-01-revised.docx) — prior revision
> - [RO-26-05-01.docx](/docs/source/RO-26-05-01.docx) — original version
>
> This page is a Markdown transcription for in-site reading.
>
> **Status:** Final, issued. **[FRAGO 26-05-01.1](frago-26-05-01-1.md) issued 21 APR 26** — adds TAK/TACN VTC, 2 MAY CG confirmation brief, 5-7 MAY Leader's Recon; does not yet address all [consistency-review](../warno-opord-review.md) items (expect a follow-on FRAGO).
>
> **Editor's note (18 APR):** The ICP facility is referenced throughout this OPORD as *"US Army Reserve Center, 399 US-11W Scenic, Mt Carmel, TN 37645"*. The official name and address is **Armed Forces Reserve Center, 399 A US-11W Scenic, Mt Carmel, TN 37645**. Use the official form in new 3 RGT products; the FRAGO will correct the OPORD wording. This is the same facility referred to as "Mt. Carmel Armory" in DIV OPORD 26-05 and "Kingsport Readiness Center" in CG's Intent.

COPY ___ OF ___
3 RGT, TNSG
Greeneville, TN
141600RAPR26

## Summary

(U) Provides advanced notice for unit training and administration involving Tennessee Maneuver Exercise 2026 (TNMAN-26).

## References

- WO 1 to DIV OPORD 26-05 (TNMAN 26)
- BG Stewart email, 30 MAR 2026, 2:32 PM
- RGT WARNO 26-05-01 TNMAN-26, dtd 112100RFEB26
- TNNG Letter of Instruction, dtd 11 DEC 24

**Time Zone:** ROMEO

## 1. Situation

See [OPORD Enclosure A — TNMAN-26 Overview (Storyboard)](opord-encl-a.md).

### Area of Interest

The states comprising FEMA Region 4 (AL, FL, GA, KY, MS, NC, SC, TN); VA; AR; MO; and the Mississippi River.

### Area of Operations

The 26 counties that comprise TEMA East Region and Home Station / Drill Locations.

### Holston Army Ammunition Plant (HAAP)

Government-owned, contractor-operated ammunition and explosives plant — the largest capacity supplier of high-performance explosives in the country.

- **Plant A** — 4509 W Stone Dr, Kingsport, TN 37660 (Sullivan County); approx. 120 acres
- **Plant B** — approx. 4 miles east in a less developed part of Hawkins County; approx. 5,900 acres
- The two plants are connected by rail. The combined site includes approx. 465 buildings.

### Enemy Forces

UNK.

### Friendly Forces

TNSG / TNNG / TEMA / Local Emergency Departments / Local Law Enforcement / TN Highway Patrol / multiple supporting states / Federal, State, and local agencies.

### Interagency, Intergovernmental, and Nongovernmental Organizations

The TEMA Eastern Regional office is established to improve services to the local emergency management directors and agencies of the several counties. Services include technical guidance, information on federal and state requirements for emergency management, updates on law and regulations, technical advice on grants, NIMS and other reports required for federal funding, information on other training, and a rapid avenue to submit requests for state or other external mutual aid or assistance.

**Major Hazards:** Several nuclear power, research, fuel, or weapons facilities; wildland/forest fires; flooding; hazardous materials; severe weather; earthquakes.

### Civil Considerations

TBD.

### Assumptions

TBD.

## 2. Mission

3 RGT participates in DIV training operations in the Tennessee Maneuver Exercise 2026 (TNMAN-26) (Simulated Kinetic Attack — Holston Ammunition Plant) in multiple locations across TN 12-17 MAY 26 IOT increase MET proficiency, sustain collective and staff training proficiency at the Regimental level.

## 3. Execution

### Initial Commander's Intent

This order synchronizes the efforts of all units throughout the Regiment IOT maintain and improve readiness and METL-based training, and Command and Control during IDT, and to prepare for TNMAN-26.

### Concept of Operations

During TNMAN-26, 3 RGT will work with TNSG / TNNG / TEMA / Local Emergency Departments / Local Law Enforcement / TN Highway Patrol / multiple supporting states / Federal, State, and local agencies to support the TNMAN-26 (Simulated Kinetic Attack at Holston Ammunition Plant Scenario Operations).

**Decisive Operation:** SECFOR Operations — Area security; Dismounted and Aerial Patrols; Traffic Control Points (TCPs); Surveillance of the maritime domain (Holston River); and Radio Communications (600 / 100 Watt) conducted in multiple locations across the AO.

This operation will be conducted in three phases.

#### Phase I — Planning and Coordination

Begins with the receipt of Warning Order #1 to DIV OPORD 26-05 (TNMAN-26), 111100RFEB26. Consists of planning, interagency and unit coordinating actions, and notifications to support tasks and actions during Phase II. Ends with the receipt of tasking from authorized unit and agency representatives which initiate exercise training actions NET 120520RMAY26.

#### Phase II — Training Execution

Begins with the receipt of tasking from authorized unit and agency representatives which initiate exercise training actions NLT 120520RMAY26 (BMNT). Consists of (but not limited to) actions which alert, mobilize, and assemble TNSG personnel, resources, and capabilities at an Incident Command Post (ICP) or Resource Staging Area in vicinity of US Army Reserve Center, 399 US-11W Scenic, Mt Carmel, TN 37645, NLT 130519RMAY26 (BMNT).

**3 MP BN** is the Focus of Main Effort (FOME) for this phase, supported by 2 MP BN, 4 MP BN, HHC 3 RGT, and other supporting agencies and units. **3 MP BN** has the Priority of Support (POS), followed by HHC 3 RGT, 2 MP BN, and 4 MP BN respectively. FOME and POS remain in effect until changed as necessary and directed by the CDR / ICP.

Phase ends with notification to end the exercise (ENDEX) and to demobilize the assembled TNSG personnel, resources, and capabilities NLT 142131RMAY26 (EENT).

#### Phase III — Recovery and AAR

Begins NET 142131RMAY26 (end of Phase II). Consists of actions required to safely, efficiently, and effectively return TNSG personnel, resources, and capabilities to respective Home of Record, Home Station, and assigned units, and to a state of preparedness / readiness to respond to contingency / emergency events (if needed). Includes post-exercise activities such as Hot Wash, After Action Review and Reporting, and Corrective Action Planning and Programming in preparation for the next training event.

Initial actions during this phase will most likely be completed within approx. +24 Hrs of ENDEX (NLT 152131RMAY26); post-training administrative actions may potentially be completed +90 Days of ENDEX (approx. 10 AUG 26).

### Tasks to Subordinate Units

#### 2 MP BN

1. **Task 1:** BPT provide personnel support to the ICP in vic US Army Reserve Center, 399 US-11W Scenic, Mt Carmel, TN 37645 NLT 120800RMAY26 in support of TNMAN-26.
2. **Task 2:** BPT provide personnel and resource support to operations of small Unmanned Aerial Systems (sUAS) assigned to reconnaissance and detection tasks in vic Holston Army Ammunition Plant, 4509 West Stone Drive, Kingsport, TN 37660-1048 NLT 120800RMAY26 in support of TNMAN-26.
3. **Task 3:** Conduct route and convoy planning to/from Home Station and the Resource Staging Area (RSA) in vic US Army Reserve Center, to be executed between 12 MAY 26 and 15 MAY 26. Planning and execution must be IAW applicable TNSG guidance and safety protocols. BPT brief the 3 RGT CDR regarding this plan NLT 302100RAPR26.

#### 3 MP BN

1. **Task 1:** BPT provide personnel support to the ICP in vic US Army Reserve Center NLT 120800RMAY26 in support of TNMAN-26.
2. **Task 2:** BPT provide personnel support to SECFOR Operations in vic Holston Army Ammunition Plant NLT 120800RMAY26 in support of TNMAN-26.
3. **Task 3:** BPT provide personnel and resource support to sUAS operations assigned to reconnaissance and detection tasks in vic Holston Army Ammunition Plant NLT 120800RMAY26 in support of TNMAN-26.
4. **Task 4:** Conduct route and convoy planning to/from Home Station and the RSA, to be executed between 12 MAY 26 and 15 MAY 26. BPT brief the 3 RGT CDR regarding this plan NLT 302100RAPR26.

#### 4 MP BN

1. **Task 1:** BPT provide personnel support to the ICP NLT 120800RMAY26 in support of TNMAN-26.
2. **Task 2:** BPT provide an FAA Sect. 107 certified UAS pilot to coordinate and support sUAS operations assigned to reconnaissance and detection tasks during TNMAN-26 NLT 120800RMAY26.
3. **Task 3:** Conduct route and convoy planning to/from Home Station and the RSA, to be executed between 12 MAY 26 and 15 MAY 26. BPT brief the 3 RGT CDR regarding this plan NLT 302100RAPR26.

#### HHC

1. **Task 1:** BPT provide personnel support to the ICP NLT 120800RMAY26 in support of TNMAN-26.
2. **Task 2:** BPT provide personnel and resource support for the establishment and operation of a Resource Staging Area (RSA) in vic US Army Reserve Center NLT 120800RMAY26 in support of TNMAN-26.
3. **Task 3:** Conduct route and convoy planning to/from Home Station and the RSA, to be executed between 12 MAY 26 and 15 MAY 26. BPT brief the 3 RGT CDR regarding this plan NLT 302100RAPR26.

#### S1

1. **Task 1:** Provide a roster of personnel committed to training in TNMAN-26 IOT facilitate Troop-to-Task organization, staffing, and MET mission sets, NLT 152000RAPR26.
2. **Task 2:** Coordinate personnel reporting requirements with DIV G1 to support TNMAN-26 NLT 152000RAPR26.
3. **Task 3:** Provide Regimental Orders to personnel participating in TNMAN-26 to respective subordinate units NLT 302000RAPR26 IOT enable employees to coordinate absence from employment prior to the order period (13-15 MAY 26).
4. **Task 4:** During TNMAN-26, track personnel names and attendance location and provide daily updates to CDR and S3 during scheduled reporting periods and as needed IOT facilitate situational awareness and support incident planning.
5. **Task 5:** BPT account for attachment of personnel from adjacent and higher units NET 120800RMAY26 IOT provide resources and capabilities for enhanced response during TNMAN-26.

#### S2

1. **Task 1:** Coordinate with DIV G2 to establish reporting requirements and provide an IPB and SA briefs in support of the Incident Command Post reporting requirements and planning purposes.

#### S3

1. **Task 1:** During TNMAN-26, BPT coordinate with external agency partners for operational support and training, to include but not limited to TEMA / TNNG.
2. **Task 2:** Provide initial estimate of Task Force Organization Structure IAW ICS protocols IOT facilitate troop-to-task planning and coordination **NLT 18 APR 26**.
3. **Task 3:** ICW S4, BPT receive medical augmentation from 61st MED BN, TNSG, to facilitate on-site medical support NLT 130800RMAY26.
4. **Task 4:** Contact and coordinate with Mr. Tony Armstrong, HAAP POC, at 423-341-5840 IOT coordinate TNSG exercise participation.
5. **Task 5:** Contact and coordinate with MAJ Trinity Callison, TNNG DJ3/5/7, exercise planner IOT coordinate TNSG exercise participation.
6. **Task 6:** Prepare a list of personnel, resources, and capabilities required to perform the assigned mission during TNMAN-26 (12-17 MAY 26) IOT request support from DIV and adjacent units.
7. **Task 7:** Prepare a Time Phased Force Deployment Data (TPFDD) as a planning tool used to organize, synchronize, and execute deployment / redeployment of TNSG personnel, resources, and capabilities to/from the RSA NLT 302100RAPR26.

#### S4

1. **Task 1:** During TNMAN-26, BPT establish a feeding, billeting, and transportation plan ICW DIV MOB Cell, DIV G4, and supporting agencies to include but not limited to TEMA / TNNG.
2. **Task 2:** ICW S3, BPT receive medical augmentation from 61st MED BN, TNSG, to facilitate on-site medical support NLT 130800RMAY26. *(Editor's note: source document still reads "Tasl" in Task 1 — minor typo.)*

#### S6

1. Prior to and during TNMAN-26, in coordination with DIV G6, receive SOI with callsigns NLT 302000RAPR26.
2. During TNMAN-26, in coordination with DIV G6, publish Signal Annex with HF / VHF frequencies and callsigns NLT 302000RAPR26.

### Coordinating Instructions

- Units BPT to submit Unit Closing Reports upon return to Home Station and verification that unit personnel have safely returned to Home of Record.
- Units review the Serious Incident Reporting requirements and protocols.

## 4. Sustainment

TBD.

## 5. Command and Control

TBP.

---

**ACKNOWLEDGE:**

ROARK
COL

**OFFICIAL:**

SHEAF
S3

**ANNEXES:** N/A.

**ENCLS:**

- [A — RO 26-05-01 HDWG TNMAN-26 Overview](opord-encl-a.md) ([source .pptx](/docs/source/ENCL-A-RO-26-05-01-HDWG-TNMAN-26.pptx))

**DISTRIBUTION:** 2 MP BN · 3 MP BN · 4 MP BN · HHC / Staff, 3 RGT
